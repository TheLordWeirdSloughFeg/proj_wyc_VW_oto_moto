...
library(sjmisc)
 ...
 str_contains(wszystkieOferty080621$`Pojemnoœæ skokowa`,",",logic=NULL)
...
#wszystkieOferty<-wszystkieOferty%>%dplyr::mutate(PojemnoÅ›Ä‡_skokowa= substr(PojemnoÅ›Ä‡_skokowa,1,str_length(PojemnoÅ›Ä‡_skokowa)-1) )
#wszystkieOferty<- wszystkieOferty%>%dplyr::mutate(PojemnoÅ›Ä‡_skokowa=as.numeric(PojemnoÅ›Ä‡_skokowa%>%str_replace_all("[^\\d,]","")%>%str_replace_all(",",".")))
wszystkieOferty<- wszystkieOferty%>%dplyr::mutate(Pojemnoœæ_skokowa=(Pojemnoœæ_skokowa%>%str_replace_all("cm3","")%>%str_replace_all("[^\\d,]","") ))
wszystkieOferty<- wszystkieOferty%>%dplyr::mutate(Pojemnoœæ_skokowa=as.numeric(Pojemnoœæ_skokowa))
...
ofertyV<-ofertyV%>%dplyr::select(-Liczba_miejsc,-Napêd)
...
tests<-subset(ofertyV,sample==FALSE)
 ...
 linearR<- lm(cena ~Przebieg+Rok_produkcji+Moc+Model_pojazdu+Skrzynia_biegÃ³w+Oferta_od,data=trains)
 predicttionsLinearR <- predict(linearR, tests) 
 myMAELinear<- mean( abs(tests$cena-predicttionsLinearR) )
 
 regrKNN<-caret::train.kknn(cena~.,data=trains,kmax=15)
 ...
 ?kknn
 plot(ppartR, uniform=TRUE,
      main="Prune Regression Tree")
 text(ppartR, use.n=TRUE, all=TRUE)
...
modelCaretRF<- caret::caret::train(Stan~.,data=trains,method="rf",metric="Accuracy")
#modelCaretRFSmall<- caret::caret::train(Stan~.,data=trainsSmall,method="rf",metric="Accuracy")

modelLM<-caret::caret::train(cena~.,data=trains,method="lm")
control<-trainControl(method = "cv",10)


modelLM<-caret::caret::train(cena~.,data=trains,method="lm",trControl=control)
...
carrpred<-predict(modelCaretRF,newdata=tests)
...
nrow(maleV) 
colnames(maleV)
names(ofertyV)<-gsub("Å›Ä‡","sc",names(ofertyV))
names(ofertyV)<-gsub("Ã³","o",names(ofertyV))
ofertyV<-ofertyV%>%droplevels()
...
cost_benchmark<- mlr::benchmark(list(learner_rf,learner_rpart,learner_kknn,learner_lm,learner_svm),resamplings = cv10,measures = mae,tasks = zadanieRegresja)
mlr::plotBMRBoxplots(cost_benchmark)
...
cost_benchmark<- mlr::benchmark(list(learner_rf,learner_rf_tuned,learner_rpart,learner_kknn,learner_lm,learner_svm),resamplings = cv3,measures = mae,tasks = zadanieRegresja)
mlr::plotBMRBoxplots(cost_benchmark,pretty.names = FALSE)

 
class(cost_benchmark) 
library(ggplot2)
require(rcompanion)
